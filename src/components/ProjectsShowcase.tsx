...
export const ProjectsShowcase: React.FC = () => {
  ...
  return (
    <section id="projects" className="py-20 relative bg-slate-950/80 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20 text-xs font-semibold mb-4 shadow-sm">
            ...
            >
              {activeTab === cat && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-gradient-to-r from-cyan-500 to-violet-600 rounded-xl shadow-lg shadow-cyan-500/20 z-0"
        ...
        {/* Projects Grid with Staggered Cascade Transition */}
        <AnimatePresence mode="wait">
          ...
          >
            {filteredProjects.map((project, idx) => (
              ...
              >
                <SpotlightCard className="h-full group relative overflow-hidden flex flex-col justify-between">
                  <div>
                    {/* Badge & Category */}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {project.techStack.map((tech) => (
                        <span
                          key={tech}
                          className="px-2 py-0.5 rounded bg-slate-900/90 text-slate-400 text-[11px] font-mono border border-slate-800"
                        >
                          {tech}
                        </span>
                      ))}
        ...
        {selectedProject && (
          ...
          >
            <motion.div
              ...
              className="glass-panel border border-slate-700 max-w-2xl w-full rounded-2xl p-6 sm:p-8 relative shadow-2xl overflow-y-auto max-h-[90vh]"
            >
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-4 right-4 p-2 text-slate-400 hover:text-white rounded-lg bg-slate-900/90 border border-slate-800 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
